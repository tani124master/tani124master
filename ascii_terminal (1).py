...
def ascii_to_svg(rows, out_path):
    ...
    total = n_rows * ROW_DURATION + PIXELS_LOOK_HOLD  # one full type-out cycle
    loop_total = total + 0.8  # small pause before it restarts
    ...
    # Invisible "master clock" — repeats forever. Every row's animation
    # re-triggers off this clock's repeat event, so the whole typing
    # sequence replays from the top each cycle instead of freezing.
    parts.append(
        f'<rect id="master" x="0" y="0" width="0" height="0" opacity="0">'
        f'<animate attributeName="opacity" from="0" to="0" '
        f'dur="{loop_total:.3f}s" begin="0s" repeatCount="indefinite"/>'
        f'</rect>'
    )
    ...
    for i, row in enumerate(rows):
        ...
        begin_times = f"0s+{start:.3f}s;master.repeatEvent+{start:.3f}s"
        ...
        parts.append(
            ...
            f'begin="{begin_times}" dur="{ROW_DURATION:.3f}s" '
            f'fill="freeze" calcMode="linear"/>'
            f'<animate attributeName="width" to="0" dur="1ms" '
            f'begin="master.repeatEvent" fill="freeze"/>'
            f'</rect>'
        )
        ...
        # then vanishes when the row is done — and resets each loop.
        row_end = start + ROW_DURATION
        end_begin_times = f"0s+{row_end:.3f}s;master.repeatEvent+{row_end:.3f}s"
        parts.append(
            f'<rect y="{y}" width="{CHAR_WIDTH:.1f}" '
            f'height="{FONT_SIZE}" fill="{CURSOR_COLOR}" opacity="0">'
            f'<animate attributeName="x" from="{PAD}" '
            f'to="{PAD + row_px:.1f}" begin="{begin_times}" '
            f'dur="{ROW_DURATION:.3f}s" fill="freeze"/>'
            f'<set attributeName="opacity" to="0.85" begin="{begin_times}"/>'
            f'<set attributeName="opacity" to="0" '
            f'begin="{end_begin_times}"/>'
            f'</rect>'
        )
